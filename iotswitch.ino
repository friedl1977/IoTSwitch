//  Project:        Listen Mode with SoftAP
...
//                  1.) Put device in listen mode when A2 is pulled to GND.
//                

//  SoftAP:         Press SETUP for 3 seconds to make the Photon enter Listening mode
//  http:           192.168.0.1
...
#define ONE_WIRE_BUS A3                 //  Temperature 3
...
int hibernate;                          //Timer - ENABLE/DISABLE whole sistem
...
BLYNK_WRITE(V11)                        // Hibernate Enable Button - turning system ON/OFF copletely
{                                       
    hibernate = param.asInt();
}
...
if (normalrun == 1 && widgetTimerEnable == 0 && hibernate == 0) {         // Do Normal Timer On stuff
        RGB.control(true);
        RGB.color(255, 0, 0);
        digitalWrite(relay, HIGH);
        Blynk.virtualWrite(V7,HIGH);
        
    }else if (normalrun == 0 && widgetTimerEnable == 0) {                   // Turn switch off
            RGB.control(false);
            digitalWrite(relay, LOW);
...
BLYNK_WRITE(V1)                         // Power Saving Mode Timer Widget 
{                                     
...
const float FullScaleAmps = 40.0;   // 20 for 10A sensor // 10 for 5A sensor // 40 for 20A sensor
...
float getRms()
{
    ...
    int ts_prev = 0;              // The timestamp of the "previous" sample
    int ts_delta = 0;             // The difference between the "present" and "previous" timestamps

    int ts_first = 0;

    int value_prev = 0;          // The integaer value of the "prevous" sample.
    int elapsedTime = 0;         // Total elapsed time for the 1000 samples
    uint32_t timeStamp = 0;      // The timestamp as read from the sensor.
